...
const DEFAULT_TAGLINE = 'STANDARDS & CERTIFICATION';
const DEFAULT_HEADING = 'Supporting compliant project delivery.';
const DEFAULT_INTRO = "PCE's registrations and certifications support execution across the project lifecycle.";
const DEFAULT_CLOSING = 'Specific certification details and applicable project documentation should be confirmed with PCE as part of the qualification or project-enquiry process.';
...
export const Certification: React.FC<CertificationProps> = ({ section }) => {
                    ...
                    <div className="flex flex-col gap-6 max-w-[540px]">
                        ...
                        <div className="flex flex-wrap items-center divide-x divide-[var(--color-hairline)] py-2 text-base md:text-[18px] font-normal tracking-wide text-[var(--color-primary)]">
                            {certifications.map((item: string, idx: number) => (
                                <span
                                    key={item}
                                    className={idx === 0 ? 'pr-4' : idx === certifications.length - 1 ? 'pl-4' : 'px-4'}
                                >
                                    {item}
                                </span>
                            ))}
