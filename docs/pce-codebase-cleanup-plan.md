...
## 5. What I did *not* touch or recommend touching
...
*Compiled from a direct read of `sanity/`, `app/`, `features/`, and `shared/` in `Projects/pce` at commit `0f7c0a4`, cross-checked with `git log` for what's changed since the last review and `madge --orphans` for an independent pass at unreferenced files.*
